...
from models.INN import DWT, IWT
...
def accuracy(output, target, topk=(1, )):
    ...
    with torch.no_grad():
        return (output.data.argmax(dim=1) == target).sum().item() / target.size(0) * 100
        ...
        with torch.no_grad():
            for name, m in self.model.named_modules():
                if isinstance(m, PassportPrivateBlock):
                    scale, _ = m.get_scale_relu()
                    signbit = scale.view(-1).sign()
                    privatebit = m.b
...
class TrainerPrivate(object):
    ...
    def train(self, e, dataloader, wm_dataloader=None, arch='resnet'):
        self.model.train()
        loss_meter = 0
        sign_loss_meter = 0
        ...
        count = 0
        count_ = 0
        w_balance = 1. #0.1 if arch == 'alexnet' else 1
        ...
        for i, (data, target) in enumerate(dataloader):
            ...
            for ind in range(2):
                ...
                if ind == 0:
                    public_acc_meter  += accuracy(pred, target)
                else:
                    private_acc_meter += accuracy(pred, target)
                    # sign loss
                    for m in self.model.modules():
                        if isinstance(m, SignLoss):
                            sign_loss += m.loss
