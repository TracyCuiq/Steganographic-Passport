...
class PassportPrivateBlock(nn.Module):
    def __init__(self, i, o, ks=3, s=1, pd=1, passport_kwargs={}):
        ...
        self.fc = nn.Sequential(
            nn.Linear(o, hid, bias=False),
            nn.LeakyReLU(inplace=True),
            nn.Linear(hid, o, bias=False),
        )
        ...
        b = custom_hash(self.skey_private, hash_length=o) # NOTE: bit information to store by using Alice signitures
    ...
    def get_scale_relu(self): #NOTE: with TLP
        ...
        scale = scale.mean(dim=0).view(1, c, 1, 1) # global average pooling value
        #NOTE: Set at this position, constrain on the conv, and the values will be rebuid by the fc
        scale_for_loss = scale * self.scale1.view(1, -1, 1, 1) #TODO:
